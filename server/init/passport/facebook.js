...
export default (passport) => {
	const facebookCb = (req, accessToken, refreshToken, profile, done) => {
		...
		User.findOne({ 'facebook.id': profile.id }, (findByFacebookErr, findByFacebookUser) => {
			console.log('1 ===> findByFacebookErr ', findByFacebookErr);
			console.log('1 ===> findByFacebookUser ', findByFacebookUser);
			if (findByFacebookUser) return done(null, findByFacebookUser);

			return User.findOne({ email: { $in: profile.emails } }, (findByEmailErr, findByEmailUser) => {
				console.log('1 ===> findByEmailErr ', findByEmailErr);
				console.log('1 ===> findByEmailUser ', findByEmailUser);
				if (findByEmailErr) { return done(findByEmailErr); }
				...
				if (!findByEmailUser) {
					...
					return done(null, findByEmailUser);
				}
			});
		});
	};
